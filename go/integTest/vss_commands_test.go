...
func (suite *GlideTestSuite) TestFtCreate() {
...
func (suite *GlideTestSuite) TestFtSearch() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
	...
	assert.Contains(suite.T(), result.Documents, prefix+"0")
	assert.Contains(suite.T(), result.Documents, prefix+"1")
	doc0, _ := result.Documents[prefix+"0"].(map[string]any)
	assert.Equal(suite.T(), vec0, doc0["vec"])
	doc1, _ := result.Documents[prefix+"1"].(map[string]any)
	assert.Equal(suite.T(), vec1, doc1["vec"])
...
func (suite *GlideTestSuite) TestFtSearchNoContent() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
	...
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), int64(2), result.TotalResults)
	// With NOCONTENT each doc value should be an empty map; both keys must be present
	assert.Equal(suite.T(), 2, len(result.Documents))
	assert.Contains(suite.T(), result.Documents, prefix+"0")
	assert.Contains(suite.T(), result.Documents, prefix+"1")
	doc0, _ := result.Documents[prefix+"0"].(map[string]any)
	assert.Empty(suite.T(), doc0)
	doc1, _ := result.Documents[prefix+"1"].(map[string]any)
	assert.Empty(suite.T(), doc1)
}

// --- ft_search_dialect ---

func (suite *GlideTestSuite) TestFtSearchDialect() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
	...
	assert.Contains(suite.T(), result.Documents, prefix+"0")
	docFields, _ := result.Documents[prefix+"0"].(map[string]any)
	assert.NotEmpty(suite.T(), docFields)
}

// --- ft_drop_and_ft_list ---

func (suite *GlideTestSuite) TestFtDropAndFtList() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtAggregateBicycles() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtAggregateMovies() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtInfo() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtAliasOperations() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtExplain() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtExplainCLI() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtSearchSortBy() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
	...
	// ASC — first result should have price=10
	...
	for _, v := range result.Documents {
		...
		p, _ := fields["price"].(string)
		prices[p] = true
	}
	assert.True(suite.T(), prices["10"])
	assert.True(suite.T(), prices["20"])
	assert.True(suite.T(), prices["30"])

	// DESC — same docs, just different order
	result, err = ft.Search(ctx, index, "@price:[1 +inf]",
...
func (suite *GlideTestSuite) TestFtSearchWithSortKeys() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
	...
	// With WITHSORTKEYS the doc map values are []any{sortKey, fieldMap}
	assert.Equal(suite.T(), 3, len(result.Documents))

	// Collect sort keys and verify all expected prices are present
	sortKeys := map[string]bool{}
	for _, v := range result.Documents {
		pair, ok := v.([]any)
		if !ok {
			suite.T().Fatalf("expected []any for WITHSORTKEYS doc value, got %T", v)
		}
		assert.Equal(suite.T(), 2, len(pair))
		sortKey, _ := pair[0].(string)
		sortKeys[sortKey] = true
		fields, _ := pair[1].(map[string]any)
		assert.NotNil(suite.T(), fields)
	}
	// Sort keys for numeric fields are prefixed with '#'; verify all three prices appear
...
func (suite *GlideTestSuite) TestFtSearchTextQueryFlags() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtSearchShardConsistency() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtAggregateQueryFlags() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtCreateIndexOptions() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtCreateFieldOptions() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtInfoWithOptions() {
	client := suite.defaultClusterClient()
	ft := client.FT()
	ctx := context.Background()
...
func (suite *GlideTestSuite) TestFtCreateStandaloneClient() {
	...
	index := "standalone-" + uuid.New().String()
	res, err := ft.Create(ctx, index, []options.Field{options.NewTextField("title")}, nil)
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), glide.OK, res)

	list, err := ft.List(ctx)
	assert.NoError(suite.T(), err)
	found := false
	for _, n := range list {
...
func (suite *GlideTestSuite) TestFtAggregateStandaloneJsonSet() {
	if len(suite.standaloneHosts) == 0 {
		suite.T().Skip("No standalone server configured")
	}
	client := suite.defaultClient()
	...
	prefix := "standalone-bikes-" + uuid.New().String() + ":"
	index := prefix + "idx"
	...
	_, err := ft.Create(ctx, index,
		...
		&options.FtCreateOptions{DataType: constants.IndexDataTypeJSON, Prefixes: []string{prefix}})
	assert.NoError(suite.T(), err)

	bikes := []string{
		`{"brand":"Velorim","price":270,"condition":"new"}`,
		`{"brand":"Bicyk","price":1200,"condition":"used"}`,
		`{"brand":"Nord","price":815,"condition":"used"}`,
	}
	for i, b := range bikes {
		suite.jsonSet(ctx, client, prefix+strconv.Itoa(i), ".", b)
	}
	time.Sleep(time.Second)

	aggreg, err := ft.Aggregate(ctx, index, "*",
		&options.FtAggregateOptions{
			Clauses: []options.FtAggregateClause{
				&options.FtAggregateGroupBy{
					Properties: []string{"@condition"},
					Reducers:   []options.FtAggregateReducer{{Function: "COUNT", Args: []string{}, Name: "count"}},
				},
			},
		})
	assert.NoError(suite.T(), err)
	assert.Equal(suite.T(), 2, len(aggreg))
	...
	assert.Equal(suite.T(), float64(1), condCounts["new"])
	assert.Equal(suite.T(), float64(2), condCounts["used"])
